    ...
    public PersonIDResult personID(String personID, String authToken){
        ...
        try {
            ...
            if (personUserName.equals(tokenUserName)){
                ...
                if (person.getFatherID() != null && !person.getFatherID().equals("")) result.setFatherID(person.getFatherID());
                result.setFirstName(person.getFirstName());
                result.setGender(person.getGender());
                result.setLastName(person.getLastName());
                if (person.getMotherID() != null && !person.getMotherID().equals("")) result.setMotherID(person.getMotherID());
                result.setPersonID(person.getPersonID());
                if (person.getSpouseID() != null && !person.getSpouseID().equals("")) result.setSpouseID(person.getSpouseID());
                result.setSuccess(true);
            }
            else throw new DataAccessException("error requested person does not belong to this user");
